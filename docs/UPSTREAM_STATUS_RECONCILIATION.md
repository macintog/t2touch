...
This historical comparison reconciles the status table inherited from
`jmurth1234/t2-touchid-linux` at upstream
commit `ea46d8a0aef3e73b0e2f747aa18721dbcd265bce` with the Linux-native E4 work
integrated into t2touch. Current installation behavior and acceptance limits are
documented in [the README](../README.md) and [validation record](RELEASE.md).
...
| ACM is only an opt-in transient-context experiment | The normal Linux-native transport service now owns ACM registration and platform metadata. Native E4 retains persistent activation material across enrollment, matching, additional enrollment, and deletion | Multi-user activation remains unproven |
...
- a blank install has a product-owned, fresh-owner identity and activation
...
- standard enrollment treats client-supplied anatomy tokens only as request
  ...
  the lock-held allocator fills the lowest vacancy without renumbering survivors;
